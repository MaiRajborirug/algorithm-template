...
class SynthradAlgorithm:
    ...
    def process_cases(self, data_path, exp_name, anatomical_region="HN", max_cases=None):
        ...
        for case_dir in tqdm(case_dirs):
            ...
            try:
                ...
                # Create comparison plot with combined metrics and info
                combined_info = {**metrics, **info}
                self.plot_comparison(
                    ...
                    metrics_info=combined_info  # Pass combined metrics and info
                )
    ...
    def calculate_metrics(self, ground_truth_ct_path, predicted_ct_path, mask_path=None):
        ...
        print(f"DEBUG: Original array shape: {original_array.shape}, range: [{np.min(original_array):.2f}, {np.max(original_array):.2f}]")
        print(f"DEBUG: Predicted array shape: {predicted_array.shape}, range: [{np.min(predicted_array):.2f}, {np.max(predicted_array):.2f}]")
        ...
        if mask_path:
            ...
            print(f"DEBUG: Mask shape: {mask_array.shape}, mask sum: {mask_bool.sum()}, total voxels: {mask_bool.size}")
            original_array = original_array[mask_bool]
            predicted_array = predicted_array[mask_bool]
            print(f"DEBUG: After masking - Original: {original_array.shape}, Predicted: {predicted_array.shape}")
        else:
            print("DEBUG: No mask provided, using full volume")
        ...
        if len(original_array.shape) == 3:
            # For 3D arrays, calculate SSIM on a middle slice or flatten
            # Option 1: Use middle slice
            mid_slice = original_array.shape[0] // 2
            try:
                ssim_score = ssim(original_array[mid_slice], predicted_array[mid_slice], 
                                 data_range=np.max(original_array[mid_slice]) - np.min(original_array[mid_slice]))
            except:
                # Fallback: use simple correlation
                ssim_score = np.corrcoef(original_array.flatten(), predicted_array.flatten())[0, 1]
        ...
        print(f"DEBUG: Calculated metrics - MAE: {mae:.6f}, PSNR: {psnr:.6f} dB, SSIM: {ssim_score:.6f}")
    ...
    def plot_comparison(self, ground_truth_ct_path, predicted_ct_path, mask_path=None, 
                       slice_indices=None, save_path=None, metrics_info=None):
        """
        ...
        Args:
            ...
            metrics_info: Dictionary containing metrics and info for title
        """
        # Load images
        original_ct = sitk.ReadImage(ground_truth_ct_path)
        predicted_ct = sitk.ReadImage(predicted_ct_path)
        ...
        # Load MRI for comparison (assuming it's in the same directory as CT)
        mri_path = ground_truth_ct_path.replace('ct.mha', 'mr.mha')
        if os.path.exists(mri_path):
            mri_image = sitk.ReadImage(mri_path)
            mri_array = sitk.GetArrayFromImage(mri_image)
        else:
            mri_array = np.zeros_like(original_array)
        ...
        # Create subplots for each view (4 columns: Predicted CT, Ground Truth CT, MRI, Mask)
        fig, axes = plt.subplots(3, 4, figsize=(20, 15))
        
        # Create title with metrics if provided
        if metrics_info:
            title = f"MAE: {metrics_info.get('MAE', 0):.4f}, PSNR: {metrics_info.get('PSNR', 0):.4f}dB, SSIM: {metrics_info.get('SSIM', 0):.4f}, Time: {metrics_info.get('elapsed_minutes', 0):.1f}min, Patches: {metrics_info.get('total_patches', 0)}, Shape: {metrics_info.get('ct_shape', 'N/A')}, Overlap: {self.overlap}"
        else:
            title = f"CT Synthesis Comparison - Overlap: {self.overlap}"
        
        fig.suptitle(title, fontsize=14)
        
        views = ['axial', 'coronal', 'sagittal']
        titles = ['Predicted CT', 'Ground Truth CT', 'MRI', 'Mask']
        
        for i, view in enumerate(views):
            if view == 'axial':
                pred_slice = predicted_array[slice_indices[view]]
                gt_slice = original_array[slice_indices[view]]
                mri_slice = mri_array[slice_indices[view]]
            ...
            # Determine value range for consistent scaling (CT images)
            ...
            axes[i, 0].axis('off')
            plt.colorbar(im1, ax=axes[i, 0], fraction=0.03, pad=0.02, shrink=0.8)
            
            # Plot ground truth CT
            im2 = axes[i, 1].imshow(gt_slice, cmap="gray", vmin=vmin, vmax=vmax)
            axes[i, 1].set_title(f"{titles[1]} ({view})")
            axes[i, 1].axis('off')
            plt.colorbar(im2, ax=axes[i, 1], fraction=0.03, pad=0.02, shrink=0.8)
            
            # Plot MRI
            mri_vmin, mri_vmax = mri_slice.min(), mri_slice.max()
            im3 = axes[i, 2].imshow(mri_slice, cmap="gray", vmin=mri_vmin, vmax=mri_vmax)
            axes[i, 2].set_title(f"{titles[2]} ({view})")
            axes[i, 2].axis('off')
            plt.colorbar(im3, ax=axes[i, 2], fraction=0.03, pad=0.02, shrink=0.8)
            
            # Plot mask
            im4 = axes[i, 3].imshow(mask_slice, cmap="gray")
            axes[i, 3].set_title(f"{titles[3]} ({view})")
            axes[i, 3].axis('off')
            plt.colorbar(im4, ax=axes[i, 3], fraction=0.03, pad=0.02, shrink=0.8)
