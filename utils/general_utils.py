...
_DESIRED_SHAPE = (4, 32, 32)
def _resize_tensor(volume: torch.Tensor, shape=_DESIRED_SHAPE) -> torch.Tensor:
    """
    Resize a 3D tensor [Z, Y, X] to [shape] using trilinear interpolation.
    """
    # Add batch and channel dims: [1, 1, Z, Y, X]
    vol = volume.unsqueeze(0).unsqueeze(0)
    vol = F.interpolate(vol, size=shape, mode="trilinear", align_corners=False)
    # Remove added dims -> [shape]
...
def load_and_prepare_data(
    ...
    for entry in data_split:
        # NOTE: change
        # map CT and MR filenames to the loader’s expected keys
        if "image" not in entry and "ct" in entry:
            entry["image"] = entry.pop("ct")
        if "mask" not in entry and "mr" in entry:
            entry["mask"] = entry.pop("mr")
        ...
        # Read CT image and convert to float32 array
        sitk_img = sitk.ReadImage(os.path.join(os.getcwd(), entry["image"]))
        np_img = sitk.GetArrayFromImage(sitk_img).astype(np.float32)  # [Z, Y, X]
        # normalize intensities (e.g., clip to typical CT window, then scale)
        np_img = np_img.clip(-1024, 3071)  # adjust window as needed
        np_img = (np_img - (3071-1024) / 2) / (3071 + 1024) / 2  # scale to [-1,1]
        
        image = torch.from_numpy(np_img)  # [Z, Y, X]      
        # resize
        image = _resize_tensor(image)
        
        # Read MR image and convert to float32 array
        sitk_mask = sitk.ReadImage(os.path.join(os.getcwd(), entry["mask"]))
        np_mask = sitk.GetArrayFromImage(sitk_mask).astype(np.float32)  # [Z, Y, X]
        np_mask = np_mask / np_mask.max()  # scale to [0,1]
        mask = torch.from_numpy(np_mask)  # [Z, Y, X]
        mask = _resize_tensor(mask)
        
        image_list.append(image)
        mask_list.append(mask)
        
        # class_list.append(entry["class"])  # each is a string
    #     class_list.append(np.zeros(_DESIRED_SHAPE[0]).astype(int))
    # Classes = class_list
    ...
    return {"images": Images, "masks": Masks, "classes": None}
